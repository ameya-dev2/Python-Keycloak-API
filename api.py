...
keycloak_connection = KeycloakOpenIDConnection(
                        ...
                        password="admin", # server admin password
...
new_user = keycloak_admin.create_user(request_obj, exist_ok=False)  # For printing the userinfo
...
roles = keycloak_admin.get_realm_roles(brief_representation=False) # Getting all the roles in Realm
role = list(filter(lambda i : i['name']== 'customer',roles))  # Getting the role info with Role Name role
role = role.pop()
print(role)

keycloak_admin.assign_realm_roles(user_id=id, roles= role) #adding roles to the user

def realm_role(role_name:str):
    roles = keycloak_admin.get_realm_roles(brief_representation=False) # Getting all the roles in Realm
    role = list(filter(lambda i : i['name']== role_name,roles))  # Getting the role info with Role Name role
    role = role.pop()
    return role  #returns the role if there is any in RoleRepresentation Format

role = realm_role("customer")
keycloak_admin.create_realm_role() # You can also create new roles for the Keycloak using create_realm_role() which uses RoleRepresention
